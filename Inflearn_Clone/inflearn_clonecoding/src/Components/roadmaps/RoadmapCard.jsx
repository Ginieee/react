...
function RoadmapCard({linkSrc, imgSrc, levelText, tagsList, title, author, lectureCnt, peopleCnt}) {
    ...
    useEffect(()=>{
        ...
        if(levelText === '입문'){
            console.log('레벨 입문 true');
            setLevel1(true);
            setLevel2(false);
            setLevel3(false);
        }
        else if(levelText === '초급'){
            console.log('레벨 초급 true');
            setLevel1(true);
            setLevel2(true);
            setLevel3(false);
        }
        else if(levelText === '중급') {
            console.log('레벨 중급 true');
            setLevel1(true);
            setLevel2(true);
            setLevel3(true);
        }
    ...
    }, [levelText, level1, level2, level3]);

    return (
        <div className='column is-3-desktop is-4-tablet is-6-mobile abtest-roadmap-card'>
            <section className='card image_card roadmap_card'>
                <Link to={linkSrc} className='e_roadmap_card_click'>
                    <div className='card-image'>
                        <figure className='image'>
                            <img src={imgSrc} alt='로드맵 대표 이미지'/>
                        </figure>
                    </div>
                </Link>
                <div className='card-content'>
                    ...
                    <div className='tags'>
                        <Link to={linkSrc} className='e_roadmap_card_click'/>
                        <div className='tag start-level'>
                            <div className='level-icon'>
                                <span className={'level-icon__step level-icon__step-1' + (level1 ? " level-icon__step--active" : "")}></span>
                                <span className={'level-icon__step level-icon__step-2' + (level2 ? " level-icon__step--active" : "")}></span>
                                <span className={'level-icon__step level-icon__step-3' + (level3 ? " level-icon__step--active" : "")}></span>
                            </div>
                            <p>{levelText}</p>
                        </div>
                        {
                            tagsList.map((item,i)=>{
                                return (
                                    <Link
                                    key={'tagsList' + i} 
                                    className='tag' to={item.linkSrc}>{item.text}</Link>
                                )
                            })
                        }
                    </div>
                    ...
                    <div className='metas'>
                        ...
                        <p className='roadmap-count meta'>{lectureCnt}</p>
                        <div className='gap mobile-line-breaker'></div>
                        <p className='enroll-count meta'>
                            <svg viewBox='0 0 16 14' fill='none' xmlns='http://www.w3.org/2000/svg' width='16' height='14'>
